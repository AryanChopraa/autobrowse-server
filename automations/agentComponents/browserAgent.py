...
def scroll_page(driver):
    try:
        ...
        # Scroll by the height of the viewport (i.e., one full page)
        driver.execute_script(f"window.scrollBy(0, {viewport_height});")
            
        # Wait for the page to settle after scrolling
        time.sleep(1)
            
        # Highlight links after scrolling
        highlight_links(driver)
    except Exception:
        pass
...
def main():
    ...
    * Save to memory: memory.add("key", "value")
    ...
    For each action you decide to take, provide an explanation of why you're taking that action, followed by the action itself. Format your response like this in a json format
    {
    ...
    messages = [
        ...
        {"role": "user", "content": "go to google serach for google stock price"}
    ]
    ...
    while True:
        ...
        if response.tool_calls:
            ...
            if tool_call.function.name == "getting_instructions_tool":
                instructions = getting_instructions_tool(arguments['query'])
                messages.append({"role": "user", "content": instructions})
            elif tool_call.function.name == "handle_typing":
                handle_typing(driver, arguments['placeholder_value'], arguments['text'])
            elif tool_call.function.name == "memory.add":
                memory.add(arguments['key'], arguments['value'])
            # elif tool_call.function.name == "memory.get":
            #     value = memory.get(arguments['key'])
            else:
                execute_action(driver, arguments['actionType'], arguments['action'])
            base64img = encode_image(SCREENSHOT_PATH)
            messages.append({
                "role": "user",
                "content": [
