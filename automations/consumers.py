...
#         # Start the screenshot sending task
#         screenshot_task = asyncio.create_task(self.send_screenshots(self.driver))
...
#                 if function_name == "handle_url":
#                     await asyncio.to_thread(handle_url, self.driver, arguments['url'])
#                 elif function_name == "handle_search":
#                     await asyncio.to_thread(handle_search, self.driver, arguments['query'])
#                 elif function_name == "handle_click":
#                     print("clicking on", arguments['text'])
#                     await asyncio.to_thread(handle_click, self.driver, arguments['text'])
#                 elif function_name == "handle_scroll":
#                     await asyncio.to_thread(handle_scroll, self.driver)
#                 elif function_name == "handle_typing":
#                     await asyncio.to_thread(handle_typing, self.driver, arguments['placeholder_value'], arguments['text'])
        ...
        async with aiohttp.ClientSession() as session:
            while True:
                ...
                if os.path.exists(SCREENSHOT_PATH):
                    ...
                    async with session.get(STARTING_SCENE_URL) as response:
                        if response.status == 200:
                            image_data = await response.read()
                            base64img = base64.b64encode(image_data).decode('utf-8')
                        else:
                            print(f"Failed to fetch starting scene image. Status: {response.status}")
                            continue
                await self.send(text_data=json.dumps({"screenshot": base64img}))
